...
OURS_RE="${OURS_RE:-orca/workspaces|$(basename "$PWD")|vite-node|vitest|puppeteer|dotnet|godot|gemini .*--model}"
...
# воркер на обрамлении, review-cut). Спека уходит в поле ввода и там остаётся:
...
" 2>/dev/null); do
    ...
    # строки, а страницы (у работающих gemini-воркеров он равен 1). Проверка по нему дала
    # четыре ложных срабатывания из четырёх. Широкий неверный страж хуже узкого
    # верного: он приучает не верить красному.
    #
    # `MCP startup incomplete` и `Use /skills` — баннер незапущенного
    # воркера, если он доходит до него ПОСЛЕ отказа MCP-серверов. Ревью
    ...
    if printf '%s' "$tail40" | grep -qE 'Gemini|Welcome to|MCP startup incomplete|Use /skills'; then
